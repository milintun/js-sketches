import { degrees, fetchJSON, floor, radians, _splineForward, XML } from './util.js';
...
export class Sketch {
  ...
  // whether to pollute the global (window) namespace with
  // variables like key and mouseX.
  _pollute = false;
  ...
  // fallback image for failed requests
  _DUMMY_IMAGE = 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';
  ...
  _handleLoad() {
    if (_isAvailable(this, 'load')) {
      ...
      Promise.all(this._loadPromises)
        .then((/*values*/) => {
          /*
          values.forEach((item) => {
  ...
  */
  ...
  quadraticVertex(cx, cy, x3, y3) {
    ...
    this.bezierVertex(x1 + ((cx-x1)*2 / 3.0), y1 + ((cy-y1)*2 / 3.0),
  ...
  */
  ...
  loadFont(location) {
    if (this._insideLoad) {
      ...
      // Step over the slash for cases where there's no slash.
      // (For instance if the URL is about:srcdoc or similar.)
      // This will leave us with a relative URL for those situations.
      ...
      if (!_openTypeLoaded()) {
        ...
        let promise = fetch(`${baseUrl}${location}`)
  ...
  textRect(str, x1, y1, x2, y2, mode=CORNER) {
    let hradius, vradius;
    ...
    let textBuffer = str + '\n';
    let length = textBuffer.length;

    let sentenceStart = 0;
    for (let i = 0; i < length; i++) {
      if (textBuffer[i] === '\n') {
        let legit =
          this._textSentence(textBuffer, sentenceStart, i, boxWidth, textBreaks);
        if (!legit) break;
        sentenceStart = i + 1;
      }
    }
    ...
    let lineX = x1;
    if (this._textAlignX == CENTER) {
      lineX = lineX + boxWidth / 2;
    } else if (this._textAlignX == RIGHT) {
      lineX = x2;
    }

    let boxHeight = y2 - y1;
    // add textAscent() for the top (baseline will be y1 + ascent)
    // and textDescent() for the bottom, so that lower parts of
    // letters are not outside the box.
    let topAndBottom = this.textAscent() + this.textDescent();
    let lineFitCount = 1 + floor((boxHeight - topAndBottom) / this._textLeading);
    let lineCount = Math.min(textBreaks.length, lineFitCount);

    if (this._textAlignY == CENTER) {
      let lineHigh = this.textAscent() + this.textLeading * (lineCount - 1);
      let y = y1 + this.textAscent() + (boxHeight - lineHigh) / 2;
      for (let i = 0; i < lineCount; i++) {
        // this._textLineAlignImpl(textBuffer, textBreaks[i], lineX, y);
        this.text(textBuffer.substring(textBreaks[i].start, textBreaks[i].stop), lineX, y);
        y += this._textLeading;
      }

    } else if (this._textAlignY == BOTTOM) {
      let y = y2 - this.textDescent() - this._textLeading * (lineCount - 1);
      for (let i = 0; i < lineCount; i++) {
        // this._textLineAlignImpl(textBuffer, textBreaks[i], lineX, y);
        this.text(textBuffer.substring(textBreaks[i].start, textBreaks[i].stop), lineX, y);
        y += this._textLeading;
   ...
   */
  _textSentence(buffer, start, stop, boxWidth, textBreaks) {
    let runningX = 0;
    ...
    while (index <= stop) {
      ...
      if ((buffer[index] == ' ') || (index == stop)) {
        ...
        if (index > wordStart) {
          ...
          wordWidth = this.textWidth(buffer.substring(wordStart, index));
        }
        ...
        if (runningX + wordWidth >= boxWidth) {
          if (runningX != 0) {
            ...
            index = wordStart;
            this._textSentenceBreak(textBreaks, lineStart, index);
            ...
            if (index - wordStart < 25) {
              ...
              while (this.textWidth(buffer.substring(wordStart, index)) < boxWidth) {
                index++;
  ...
  _textSentenceBreak(breaks, start, stop) {
    breaks.push({
  ...
  */
  ...
  loadStrings(location) {
    if (this._insideLoad) {
      ...
      console.error(`loadStrings() can only be used inside load(), use fetchStrings() anywhere else`);
      return null;
    }
  }


  loadTable(location, ...options) {
    if (this._insideLoad) {
      let outgoing = new Table();
      let promise = fetch(location)
        .then((response) => response.text())
        .then((text) => {
          outgoing.init(text, ...options);
        })
      this._loadPromises.push(promise);
      return outgoing;

    } else {
      console.error(`loadTable() can only be used inside load(), use fetchTable() anywhere else`);
      return null;
    }
  }
    ...
    if (this._insideLoad) {
      ...
      let promise = new Promise((resolve, reject) => {
        ...
        img.onerror = () => {
          ...
          // console.error(error);
          let msg = `Could not load image from ${location}. Make sure the image exists and can be opened.`;
          // reject(new Error(msg));
          // console.error(msg, error);
          console.error(msg);
          // replace with transparent dummy image to cause less trouble
          img.src = this._DUMMY_IMAGE;
          resolve(img);
        };
  ...
  matte(color) {
    ...
    if (this._ratioTop != 0) {
      ...
      this.ctx.fillRect(0, this.height - this._ratioTop, this.width, this._ratioTop);  // bottom
    }
  ...
  */
  ...
  _setupCanvas(canvasId) {
    ...
    this.canvas.addEventListener('pointerdown', function(event) {
      ...
      // Pick up events outside the canvas during drag, etc
    ...
    this.canvas.addEventListener('pointercancel', function(event) {
      sketch.canvas.releasePointerCapture(event.pointerId);
    }, false);
    ...
    /*
    // didn't work: tabIndex was set but still no events
    // allow focus for a canvas
    this.canvas.tabIndex = 0;  // focus help?
    this.canvas.tabIndex = '1';
    // remove the border when focused
    this.canvas.style.outline = 'none';

    this.canvas.addEventListener("keydown", (event) => {
      console.log('event on canvas', event);
    });
    */
    ...
    window.addEventListener("keydown", (event) => {
      if (this.focused) {
        if (!event.repeat) {
          if (_isAvailable(sketch, 'keyPressed')) {
            sketch.key = event.key;
            if (sketch._pollute) {
            // if (Object.hasOwn(window, 'key')) {
              window.key = event.key;  // if we're using wheels
            }
            // pass ESC up to the editor
            if (window.parent && event.key == 'Escape') {
              if (Object.hasOwn(window.parent, 'stop')) {
                window.parent.stop();
              }
            }
    ...
    window.addEventListener("keyup", (event) => {
      if (!event.repeat) {
        if (_isAvailable(sketch, 'keyReleased')) {
          sketch.key = event.key;
          if (sketch._pollute) {
          // if (Object.hasOwn(window, 'key')) {
            window.key = event.key;  // if we're using wheels
