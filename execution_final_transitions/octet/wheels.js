...
function initWheels() {
  if (_wheels != null) {
    ...
    class TrainingSketch extends Sketch {
      ...
      // makes sure that values are set for all these on 'window'
      _polluteOnce() {
        this._pollute = true;  // do more of this later
      ...
      _handleDraw(when) {
        ...
        // TODO only update when actually changed
        if (this._ratio) {
          window.width = this.rwidth;
          window.height = this.rheight;
        } else {
        ...
        // need to update more frequently than each frame
        // this._polluteMouse();
        // this._polluteKey();
    ...
    */
    ...
    if (typeof(keyPressed) !== 'undefined') _wheels.keyPressed = keyPressed;
    if (typeof(keyReleased) !== 'undefined') _wheels.keyReleased = keyReleased;
